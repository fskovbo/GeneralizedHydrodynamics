...
methods (Access = protected)
    ...
    function dT = calcScatteringRapidDeriv(obj, t, x, rapid1, rapid2, type1, type2)
        % for now: rapid1 is 1st dim, rapid2 is 2nd dim
        if size(rapid1,2) ~= 1 && numel(rapid1) ~= 1
            rapid1 = permute(rapid1, [2 1 3]);
        end
        if size(rapid2,1) ~= 1 && numel(rapid2) ~= 1
    ...
    function [v_eff, a_eff] = calcEffectiveVelocities(obj, theta, t, x, rapid, type)        
        ...
        if ~isempty(obj.couplings.dcdt) || ~isempty(obj.couplings.dcdx)
            ...
            dTdc    = 2*(rapid - permute(obj.rapid_grid,[2 1]))./(obj.couplings.c(t,x).^2 + (rapid - permute(obj.rapid_grid,[2 1])).^2);
            dTdc(isnan(dTdc)) = 0; % removes any NaN
            B       = delta_k/(2*pi) * dTdc.*transpose(theta);
        end
