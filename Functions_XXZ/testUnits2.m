...
for i = 1:6
    ...
    rho_HO = 2*(hbar_si/m_si)*rho_t_HO{index}*1e9; % [ms/um^2]
    rho_AHO = 2*(hbar_si/m_si)*rho_t_AHO{index}*1e9; % [ms/um^2]
    
    % plot Harmonic quasiparticle distribution
    subplot(3,6,i)
    imagesc(x_array*1e6, v_array*1e3,squeeze(double(rho_HO)))
    set(gca,'YDir','normal')
    colormap(hot)
    caxis([0 0.36])
    ylabel('v [um/ms]')
    ...
    subplot(3,6,i+6)
    imagesc(x_array*1e6, v_array*1e3,squeeze(double(rho_AHO)))
    set(gca,'YDir','normal')
    colormap(hot)
    caxis([0 0.36])
    ylabel('v [um/ms]')
...
function theta_bragg = applyBraggPulse(theta, k_Bragg, dk, LLS)
    ...
    rho_mat     = double(rho);
    rho_shift   = circshift(permute(rho_mat/2, [5 1 2 3 4]), [0, round(k_Bragg/dk)]);
    rho_bragg   = permute(rho_shift + fliplr(rho_shift) , [2 3 4 5 1]);
    rho_bragg   = GHDtensor(rho_bragg);
